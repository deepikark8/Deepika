package createaccount;

import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.chrome.ChromeDriver;
import org.openqa.selenium.support.ui.ExpectedConditions;
import org.openqa.selenium.support.ui.Select;
import org.openqa.selenium.support.ui.WebDriverWait;

import io.github.bonigarcia.wdm.WebDriverManager;

public class TC12 {
	static WebDriver driver; 
	
	public static void main(String[] args) throws Exception {
		login();
		account();
		viewDropDown();
		editView();
		
	}

	
	public static void login() {
		
		
		WebDriverManager.chromedriver().setup();
		driver = new ChromeDriver();
		driver.get("https://teka-dev-ed.develop.my.salesforce.com/");
		
		WebElement username = driver.findElement(By.id("username"));
		username.sendKeys("deepika@mh.com");
		
		WebElement password = driver.findElement(By.id("password"));
		password.sendKeys("password@123");
		
		
		WebElement login = driver.findElement(By.xpath("//input[@type='submit']"));
		login.click();
		
		
		
		}
	
	public static void account() throws InterruptedException {
		Thread.sleep(5000);
		//WebElement elemnet = driver.findElement(By.xpath("//a[@class='listRelatedObject accountBlock title']"));
		
		WebElement elemnet = driver.findElement(By.xpath("//a[@title='Accounts Tab']"));
		
		waitforVisibility(elemnet,5);
		
		driver.findElement(By.id("tryLexDialogX")).click();
		
	}
	
	public static void viewDropDown() {
		
		WebElement element = driver.findElement(By.id("fcf"));
		Select select = new Select(element);
		select.selectByVisibleText("Epic1");
		
		driver.findElement(By.xpath("//a[text()='Edit']")).click();
		
	}
	
	public static void editView() {
		
		WebElement viewName = driver.findElement(By.id("fname"));
		viewName.clear();
		viewName.sendKeys("Epic");
		
		WebElement filterAccountName = driver.findElement(By.id("fcol1"));
		Select select = new Select(filterAccountName);
		select.selectByVisibleText("Account Name");
		
		
		WebElement filterOperator= driver.findElement(By.id("fop1"));
		Select select1 = new Select(filterOperator);
		select1.selectByVisibleText("contains");
		
		driver.findElement(By.id("fval1")).sendKeys("a");;
		
		
		WebElement fieldToDisplay= driver.findElement(By.id("colselector_select_0"));
		Select select2 = new Select(fieldToDisplay);
		select2.selectByVisibleText("Last Activity");
		
		driver.findElement(By.xpath("//img[@alt='Add']")).click();
		
		driver.findElement(By.name("save")).click();
	}
	
	//wait
	
	public static void waitforVisibility(WebElement element, int time) {
		
		
		
		WebDriverWait wait = new WebDriverWait(driver,time);
		wait.until(ExpectedConditions.visibilityOf(element));
		element.click();
	}
}
